...
\begin{equation}\label{方程}
    ...
    \end{cases}
\end{equation}
其中$x$是$\realset^d$值未知函数，$x_0\in \realset^d$给定，$0<\alpha<1$, 函数$f:[0,\infty)\times \realset^d\times \realset^d$连续，$\Caputo x(t)=\Gamma(1-\alpha)^{-1}\int_{0}^{t}(t-s)^{-\alpha}x'(s)\differential s$. 参考\cite{mild}, 我们给出对于\mainEquation 的弱解的定义。
\begin{definition}
